...
const Allergy = (params: any) => {
  ...
  const getAllergy = () => {
    axios
      .get(config.baseURL + "/api/meals/allergyitems")
      .then(response => {
        console.log("response--------", response?.data?.dislikes)
  ...
  const onNextButtonPress = () => {
    var array_for_allergy: any = []
    for (let i = 0; i < selectAllergy?.length; i++) {
      array_for_allergy.push(selectAllergy[i]?.value)
    }
    // disliked_item_ids
    // disliked_meal_ids
    // allergy_item_ids
    const registerData = params.route.params?.registerData
    registerData.allergy_item_ids = array_for_allergy
    params.navigation.navigate("Address", { registerData: registerData })
  }

  const renderItem = ({ item, index }: { item: any; index: number }) => {
    let selectedItem: boolean =
      [...selectAllergy].findIndex((e: any) => e.value === item.value) != -1
    const setArray = (item: any) => {
      var _Array: any = selectAllergy
      const find = [...selectAllergy].findIndex((e: any) => e.value === item.value)
      find == -1 ? _Array.push(item) : _Array.splice(find, 1)
      setSelectAllergy([..._Array])
    }
    return (
    ...
    <View style={styles.body}>
      <SafeAreaView
        ...
        <View style={styles.headingInnerStyle}>
          ...
          <Text preset="button01" color={colors.white}>
            {lang[lang.lang].personal_profile_125}
          </Text>
          ...
          <View style={styles.arrowDownContainer}>
            <View style={styles.arrowDownListContainer}>
              {selectAllergy?.length >= 1 ? (
                ...
                <View>
                  <Text text={lang[lang.lang].personal_profile_1255} preset="body" color={"#999"} />
                </View>
              )}
            </View>
          </View>
      ...
      <View style={styles.buttonsContainer}>
        <TouchableOpacity
          ...
          onPress={() =>
            params.navigation.navigate("Address", {
