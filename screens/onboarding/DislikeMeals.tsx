...
const DislikeMeals = (params: any) => {
  console.log("params.route.params?.registerData----", params.route.params?.registerData)
  ...
  const [selectMealsDisliked, setSelectMealsDisliked] = useState<any[]>([])

  const getDislikeMeals = () => {
    axios
      .get(config.baseURL + "/api/meals/allmeals")
      .then(response => {
  ...
  const onNextButtonPress = () => {
    var array_for_disliked_meal: any = []
    for (let i = 0; i < selectMealsDisliked?.length; i++) {
      array_for_disliked_meal.push(selectMealsDisliked[i]?.value)
    }
    // disliked_item_ids
    // disliked_meal_ids
    // allergy_item_ids
    const registerData = params.route.params?.registerData
    registerData.disliked_meal_ids = array_for_disliked_meal
    params.navigation.navigate("Allergy", { registerData: registerData })
  }

  const renderItem = ({ item, index }: { item: any; index: number }) => {
    let selectedItem: boolean =
      [...selectMealsDisliked].findIndex((e: any) => e.value === item.value) != -1
    const setArray = (item: any) => {
      var _Array: any = selectMealsDisliked
      const find = [...selectMealsDisliked].findIndex((e: any) => e.value === item.value)
      find == -1 ? _Array.push(item) : _Array.splice(find, 1)
      setSelectMealsDisliked([..._Array])
    }
    return (
    ...
    <View style={styles.body}>
      <SafeAreaView
        ...
        <View style={styles.headingInnerStyle}>
          ...
          <Text preset="button01" color={colors.white}>
            {lang[lang.lang].personal_profile_124}
          </Text>
      ...
      <View style={{ height: Platform.OS == "android" ? "88%" : "80%" }}>
        <View style={{ paddingHorizontal: spacing.medium }}>
          <Text text={lang[lang.lang].personal_profile_1244} preset="t2" />
        </View>
        <View style={styles.listSelectedContainer}>
          <View style={styles.arrowDownContainer}>
            <View style={styles.arrowDownListContainer}>
              {selectMealsDisliked?.length >= 1 ? (
                <View>
                  <FlatList
                    horizontal
                    showsVerticalScrollIndicator={false}
                    data={selectMealsDisliked}
                ...
                <View>
                  <Text text={lang[lang.lang].personal_profile_1244} preset="body" color={"#999"} />
                </View>
              )}
            </View>
          </View>
      ...
      <View style={styles.buttonsContainer}>
        <TouchableOpacity
          ...
          onPress={() =>
            params.navigation.navigate("Allergy", {
              registerData: params.route.params?.registerData,
            })
          }>
          <Text preset="button01" color={colors.white}>
            {lang[lang.lang].complete_profile_7}
          </Text>
        </TouchableOpacity>
